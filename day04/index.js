...
const pt1 = lines => lines.reduce((pr, line) => { const [[a1, a2], [b1, b2]] = line.split(",").map(x => x.split("-").map(Number)); return ((a1 >= b1 && a2 <= b2) || (b1 >= a1 && b2 <= a2)) ? pr + 1 : pr }, 0)
const pt2 = lines => lines.reduce((pr, line) => { const [[a1, a2], [b1, b2]] = line.split(",").map(x => x.split("-").map(Number)); return ((a1 >= b1 && a1 <= b2) || (a2 >= b1 && a2 <= b2)) || ((b1 >= a1 && b1 <= a2) || (b2 >= a1 && b2 <= a2)) ? pr + 1 : pr }, 0)
